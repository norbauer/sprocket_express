      ...
      orders.each do |order|
        ...
        while unassigned_groupings_of_five_or_fewer_products.size > 0
          ...
          if first_row_in_the_current_order
            ...
            # continuation lines (preposterously) require a shipping address to be specified, unless billing_same_as_shipping
            unless order.shipping_same_as_billing?
              ['last_name','first_name','company','address_1','address_2','city','state','zipcode','country','phone','email'].each do |attribute_suffix|
                attribute = 'shipping_' + attribute_suffix
                row_to_add[SprocketExpress::order_attributes_to_csv_column_names[attribute.intern]] = order.send(attribute)
              end
            end
          end
